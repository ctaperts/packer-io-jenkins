...
cd /root/ && java -jar `locate jenkins-cli.jar` -s http://localhost:8080/ install-plugin git github github-pullrequest github-branch-source email-ext docker-plugin --username $JENKINS_USERNAME --password $JENKINS_PASSWORD
